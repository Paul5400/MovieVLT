  ...
  <div class="p-4 text-white">
    ...
    <div v-if="movies && movies.length"
      ...
      <div
        ...
        class="bg-gray-800 rounded-lg shadow-lg overflow-hidden hover:shadow-2xl transition-shadow"
      >
        <img
        ...
        <div class="p-4">
          ...
          <p class="text-sm text-gray-300">Année : {{ fixYear(movie.Year) }}</p>
        </div>
      </div>
    </div>
  </div>
    ...
    async fetchMovies() {
      ...
      try {
        const response = await fetch(
          `https://www.omdbapi.com/?apikey=${this.apiKey}&s=${encodeURIComponent(this.searchQuery)}&y=${this.searchQuery}`
        );
        if (!response.ok) {
